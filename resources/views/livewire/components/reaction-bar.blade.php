...
<div class="flex items-center gap-2" @click.stop>
    <div x-data="{ open: false }" class="relative">
        <button
            type="button"
            @click="open = !open"
            class="group flex items-center gap-1.5 rounded-full px-2 py-1.5 text-sm font-medium ring-1 ring-transparent transition-all duration-200 {{ $this->currentReaction ? 'text-pink-600 dark:text-pink-400 bg-pink-50/60 dark:bg-pink-900/30 ring-pink-200/60 dark:ring-pink-800/60 hover:bg-pink-50 dark:hover:bg-pink-900/40' : 'text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-800 dark:hover:text-zinc-200 hover:ring-zinc-200/60 dark:hover:ring-zinc-700/60' }}"
        >
            <div class="p-1 rounded-full transition-all duration-200 {{ $this->currentReaction ? 'bg-pink-50 dark:bg-pink-900/30' : 'group-hover:bg-zinc-200/40 dark:group-hover:bg-zinc-700/40' }}">
            ...
            <span class="transition-all duration-200 {{ $this->currentReaction ? '' : 'group-hover:text-zinc-800 dark:group-hover:text-zinc-200' }}">
            ...
            <div class="flex flex-col gap-1">
                @foreach (['like', 'love', 'laugh', 'wow', 'sad', 'angry'] as $type)
                    <button
                        ...
                        @click="open = false"
                        class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-sm font-medium transition-all duration-200 {{ $this->currentReaction === $type ? 'bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 ring-1 ring-pink-200/70 dark:ring-pink-800/70' : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 hover:ring-1 hover:ring-zinc-200/70 dark:hover:ring-zinc-600/70' }}"
                    >
